// src/modules/financeiro/components/Despesas.tsx
...
const CATEGORIAS = [
  ...
  { value: "ajuda_social", label: "Ajuda Social/Cáritas" },
  { value: "formacao", label: "Formação/Cursos" },
  { value: "alimentacao", label: "Alimentação" },
  { value: "transporte", label: "Transporte" },
  { value: "salario_gratificacao", label: "Salários/Gratificações" },
  { value: "repasse_diocese", label: "Repasse à Diocese" },
...
export const Despesas: React.FC = () => {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const r = await db.select<Lancamento[]>(
        "SELECT id, descricao, valor, data, categoria, origem FROM lancamentos WHERE tipo='SAIDA' AND deleted_at IS NULL ORDER BY data DESC, id DESC LIMIT 100"
      );
      setLista(r);
  ...
  const salvar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDescricao(""); setValor(""); setCategoria("manutencao");
      await carregar();
      setAlerta({ tipo: "sucesso", msg: "Despesa registrada!" });
    } catch { setAlerta({ tipo: "erro", msg: "Erro ao salvar despesa." }); }
  };
  ...
  return (
    <div>
      <ModalConfirm aberto={idExcluir !== null} titulo="Excluir Despesa" mensagem="Deseja excluir este lançamento permanentemente?" onConfirmar={confirmarExcluir} onCancelar={() => setIdExcluir(null)} />
      ...
      <div style={{ display: "flex", gap: spacing.lg, alignItems: "flex-start" }}>
        <div style={{ width: 320, background: colors.surface, borderRadius: 14, border: `1px solid ${colors.border}`, padding: spacing.lg, flexShrink: 0 }}>
          <h3 style={{ margin: `0 0 ${spacing.md}px`, fontSize: typography.fontSize.sm, fontWeight: 700, color: colors.danger }}>− Nova Despesa</h3>
          <form onSubmit={salvar} style={{ display: "flex", flexDirection: "column", gap: spacing.sm }}>
            ...
            <input type="text" value={descricao} onChange={e => setDescricao(e.target.value)} placeholder="Ex: Conta de energia mês..." style={inS} required />
            ...
            <button type="submit" style={{ marginTop: 4, padding: "10px", background: colors.danger, color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer", fontFamily: typography.fontFamily }}>
              Salvar Despesa
            </button>
          </form>
        </div>
        ...
        <div style={{ flex: 1, background: colors.surface, borderRadius: 14, border: `1px solid ${colors.border}`, overflow: "hidden" }}>
          <div style={{ padding: `${spacing.md}px ${spacing.lg}px`, borderBottom: `1px solid ${colors.divider}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontWeight: 700, fontSize: typography.fontSize.sm }}>Despesas Registradas ({lista.length})</span>
            <span style={{ fontWeight: 800, color: colors.danger, fontSize: typography.fontSize.sm }}>Total: R$ {total.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>
          </div>
          {carregando ? <div style={{ padding: spacing.xl, textAlign: "center", color: colors.textMuted }}>Carregando...</div> : lista.length === 0 ? (
            <div style={{ padding: spacing.xl, textAlign: "center", color: colors.textMuted }}>Nenhuma despesa registrada.</div>
          ) : (
                ...
                <tbody>
                  {lista.map(l => (
                    <tr
                      ...
                      onClick={() => selectItem({ type: "lancamento", tipoLanc: "SAIDA", id: l.id, valor: l.valor, descricao: l.descricao, data: l.data, categoria: l.categoria, origem: l.origem })}
                    >
                      ...
                      <td style={{ padding: "10px 14px" }}><span style={{ background: "#fee2e2", color: "#991b1b", padding: "2px 8px", borderRadius: 6, fontSize: typography.fontSize.xs, fontWeight: 600 }}>{CATEGORIAS.find(c => c.value === l.categoria)?.label ?? l.categoria}</span></td>
                      <td style={{ padding: "10px 14px", fontSize: typography.fontSize.sm, color: colors.textSecondary }}>{l.origem}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: colors.danger }}>R$ {l.valor.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</td>
